...
<div><h3 class="text-sm font-semibold mb-1">Paquete para el contador</h3><p class="text-xs text-ink-subtle mb-2">ZIP con gastos.xlsx, cobros.xlsx, pagos.xlsx, nomina.xlsx${esAdmin() ? ', retiros_socios.xlsx' : ''}, resumen.pdf y la carpeta xml/ del mes.</p>
<div class="flex flex-wrap gap-2"><button type="button" class="btn btn-s text-sm" onclick="Cierres.paquete('${p}')"><i class="ri-download-2-line" aria-hidden="true"></i> Generar paquete</button>${esAdmin() ? (cerrado ? `<button type="button" class="btn btn-s text-sm" onclick="Cierres.reabrir('${p}')"><i class="ri-lock-unlock-line" aria-hidden="true"></i> Reabrir mes</button>` : `<button type="button" class="btn btn-p text-sm" onclick="Cierres.cerrar('${p}')"><i class="ri-lock-line" aria-hidden="true"></i> Cerrar ${mesLabel(p).split(' ')[0]}</button>`) : ''}${ci?.paquete_path ? `<button type="button" class="btn btn-s text-sm" onclick="Cierres.enlace('${p}')"><i class="ri-link" aria-hidden="true"></i> Copiar enlace del paquete (7 días)</button>` : ''}</div>
